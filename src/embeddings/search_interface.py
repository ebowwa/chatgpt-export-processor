...
class ConversationSearcher:
    ...
    def __init__(
        ...
        Args:
            ...
            embeddings_model: Ollama model name (must match model used for embeddings)
        """
    ...
    def _load_embeddings(self):
        ...
        logger.info(f"Loaded {len(self.chunks):,} conversation embeddings")
        logger.info(f"Total conversations processed: {self.metadata['total_conversations']:,}")
    ...
    def find_similar_to_conversation(self, conversation_id: str, top_k: int = 10) -> List[Tuple[float, Dict[str, Any]]]:
...
def main():
    parser = argparse.ArgumentParser(
        description="Search through ChatGPT conversation embeddings"
    )
    parser.add_argument(
        "embeddings_dir",
